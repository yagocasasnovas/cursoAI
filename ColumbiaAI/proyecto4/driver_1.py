...
def forward_checking2(assign):
	a = 1
	for kk in letters:
		for nn in numbers:
			code = kk+nn
			if len(domains[code]) == 0:
				a = 0
	return a
...
def backtrack(assignment):
	...
	for value in order_domain_values_random(var1):
		if check_consistency(var1,value,assignment) == 1:
			"print str(var1)+ ' ' + str(value) + str(domains[var1])"
			assignment[var1] = value
			forward_checking2[assignment]
			"pretty_print(assignment)"
			"""raw_input('press key')"""
...
for sudok in list_of_sudokus:
	...
	"""AC3"""
	"crear cola de arcos -- set sin duplicados"
	kao = 0
	arc_queue = create_initial_queue(sudok_AC3)
	
	domains = create_domains(sudok_AC3)
	
	"print 'inicio AC3 ' + str(rr)"
	"pretty_print(sudok)"
	...
	while len(arc_queue) != 0:
		...
		if arc[0] == 'A6':
			"print str(ii) + ' ' + str(domains[arc[0]]) + ' ' + str(arc[1]) + ' ' + str(domains[arc[1]]) + ' val ' + str(sudok[arc[1]])"

			"""raw_input('press key')"""
			ii = ii + 1
		
		
		if revise_arc(arc,sudok_AC3,domains) == 1:
