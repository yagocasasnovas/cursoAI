...
with open('sudokus_start2.txt') as f:
    content = f.readlines()
...
def pretty_print_domain(sak):
	
	for ll in numbers:
		for ss in letters:
			print ss+ll + ': ' + str(sak[ss+ll])
...
def update_domains2(var,val):
	...
	for nn in numbers:
		if nn != numero:
			xj = letra+nn
			if val not in domains_BTS[xj]:
				domains_BTS[xj].append(val)
	...
	for kk in letters:
		for nn in numbers:
			code = kk+nn
			if code != var and cubo == get_cube(kk,nn):
				if val not in domains_BTS[code]:
					domains_BTS[code].append(val)
...
def forward_checking(assignment):
	...
	for nn1 in numbers:
		for kk1 in letters:
			code = kk1+nn1
			val = assignment[code]
			cubo = get_cube(kk1,nn1)
			"para cada valor si es distinto de cero actualizo sus directos"
			...
			if val != 0:
				...
				"lineas"
				for nn in numbers:
					if nn != numero and assignment[letra+nn] == 0:
						if val in domains_BTS[letra+nn]:
							domains_BTS[letra+nn].remove(val)
				


				"columnas"
				for kk in letters:
					if kk != letra and assignment[kk+numero] == 0:
						if val in domains_BTS[kk+numero]:
							domains_BTS[kk+numero].remove(val)
				...
				for kk in letters:
					for nn in numbers:
						code1 = kk+nn
						if code1 != code and cubo == get_cube(kk,nn) and assignment[code1] == 0:
							if val in domains_BTS[code1]:
								domains_BTS[code1].remove(val)
								
			else:
				domains_BTS[code] = ['1','2','3','4','5','6','7','8','9']
...
def backtrack(assignment):
	...
	for value in domains_BTS[var1]:
		if check_consistency(var1,value,assignment) == 1:
			...
			"""pretty_print(assignment)
			pretty_print_domain(domains_BTS)"""
