...
class PriorityQueue:
	...
	def isEmpty(self):
 	...
 	def push(self, item , priority):
 		...
 		heapq.heappush(self.nodes, (priority, item))
...
if SearchAlgorithm == 'ast':
	...
	while stack:
		...
		nodeValueTuple = stack.pop()

		nodeValue = nodeValueTuple[2]
		...
		if nodeValue == GoalState:
			...
			print 'path_to_goal: ' + str(route.list)
			print 'cost_of_path: ' + str(route.isLong())
			print 'nodes_expanded: ' + str(len(closed_nodes))
		...
		if nodeValue not in closed_nodes:
			...
			for childnode in GetPossibleMovementsAST(nodeValue,board_dimension):
				...
				if childnode[1] in open_nodes and cost_temp < dict_nodes[childnode[1]][1]:
					

					fcost = cost_temp + ManhattanDistance(childnode[1],GoalState,board_dimension)
					node_id = node_id + 1
					"""print ' - ' + str(fcost) + ' ' + str(dict_nodes[childnode[1]][0])"""
															
															
					dict_nodes[childnode[1]][0] = fcost
					dict_nodes[childnode[1]][1] = cost_temp
					dict_nodes[childnode[1]][3] = (nodeValue,childnode[0])
					dict_nodes[childnode[1]][2] = dict_nodes[nodeValue][2] + 1

					childnodeTuple = (udlr_to_number(childnode[0]),node_id, childnode[1])

					stack.update(childnodeTuple, fcost)
				...
				if childnode[1] not in open_nodes and childnode[1] not in closed_nodes:
					...
					"""establezco coste"""
 					dict_nodes[childnode[1]] = [0,cost_temp,0,0,node_id,childnode[0]]
 					...
 					"""creo tuple e inserto en stack"""
 					childnodeTuple = (udlr_to_number(childnode[0]), node_id, childnode[1])
 					"""print childnodeTuple"""
					stack.push(childnodeTuple,fcost)
					...
					"""print 'Direction: ' + str(childnode[0])"""
					"""print_cube(childnode[1])"""
					"""print 'Cost: ' + str(cost_temp)"""
					"""print 'Manhattan: ' + str(ManhattanDistance(childnode[1],GoalState,board_dimension))"""
